    ...
    public ApplicationLayer(String pName) {
        ...
        item_delete_button.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                ...
                if (delete_ip != null) {
                    if (((ARPLayer) m_layer_mgr.getLayer("ARP")).cache_table.containsKey(delete_ip)) {
                        Object[] value = ((ARPLayer) m_layer_mgr.getLayer("ARP")).cache_table.get(delete_ip);
                        if (System.currentTimeMillis() - (long) value[3] / 1000 > 1) {
                            // cache table에서 입력한 ip주소에 해당하는 값 제거
                            ((ARPLayer) m_layer_mgr.getLayer("ARP")).cache_table.remove(delete_ip);
                            ((ARPLayer) m_layer_mgr.getLayer("ARP")).updateCacheTable();
                        }
                    }
                }
            }
        ...
        all_item_delete_button.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                Set key = ((ARPLayer) m_layer_mgr.getLayer("ARP")).cache_table.keySet();
                ArrayList<String> delete_key = new ArrayList<String>();
                for (Iterator iterator = key.iterator(); iterator.hasNext(); ) {
                    String key_value = (String) iterator.next();
                    Object[] value = ((ARPLayer) m_layer_mgr.getLayer("ARP")).cache_table.get(key_value);
                ...
                for (int i = 0; i < delete_key.size(); i++)
                    ((ARPLayer) m_layer_mgr.getLayer("ARP")).cache_table.remove(delete_key.get(i));
                ((ARPLayer) m_layer_mgr.getLayer("ARP")).updateCacheTable();
            }
        });
        ...
        arp_send_button.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                ...
                if (arp_ip_address_textfield.getText() != "") {
                    ...
                    // IP계층의 Send함수 호출
//					(IPLayer)getUnderLayer().send(bytes, bytes.length);
                    ((IPLayer) m_layer_mgr.getLayer("IP")).send(bytes, bytes.length);
                } else {
        ...
        btnAdd.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                HashMap<String, Object[]> proxyTable = ((ARPLayer) m_layer_mgr.getLayer("ARP")).proxy_table;
        ...
        arp_delete_button.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                ...
                if (delete_ip != null) {
                    if (((ARPLayer) m_layer_mgr.getLayer("ARP")).proxy_table.containsKey(delete_ip)) {
                        // 테이블에서 제거
                        ((ARPLayer) m_layer_mgr.getLayer("ARP")).proxy_table.remove(delete_ip);

                        // 삭제 후 Proxy table 출력창 업데이트
                        // cache table은 ARP계층에 updateARPCacheTable함수가 있었지만 proxy table업데이트 함수는 없음
                        String print_result = "";
                        for (Iterator iterator = ((ARPLayer) m_layer_mgr.getLayer("ARP")).proxy_table.keySet().iterator(); iterator.hasNext(); ) {
                            String key_ip = (String) iterator.next();
                            Object[] obj = ((ARPLayer) m_layer_mgr.getLayer("ARP")).proxy_table.get(key_ip);
                            print_result = print_result + "    " + (String) obj[0] + "\t";
                            byte[] mac = (byte[]) ((ARPLayer) m_layer_mgr.getLayer("ARP")).proxy_table.get(key_ip)[1];
                            String ip_String = key_ip;
                            String mac_String = "";
                        ...
                        int proxy_size = ((ARPLayer) m_layer_mgr.getLayer("ARP")).proxy_table.size();
                        proxy_area.setText(print_result);
                    }
                }
            }
        ...
        garp_send_button.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                ...
                if (hardware_textfield.getText() != "") {
                    ...
                    //IP계층의 Send함수 호출
//	        		 getUnderLayer().send(hwAddress, hwAddress.length, "GARP");
                    ...
                    if (ethernet_src_address.getText().compareTo("") != 0 && ip_src_address.getText().compareTo("") != 0) {
                        ...
                        ((ARPLayer) m_layer_mgr.getLayer("ARP")).setHostMacAddr(ethernet_src);    // 자기 자신의 mac 주소 설정

                        ((IPLayer) m_layer_mgr.getLayer("IP")).setIPSrcAddress(ip_src);            // 자기 자신의 ip 주소 설정
...
//	        	           	((NILayer) m_layer_mgr.getLayer("NI")).SetAdapterNumber(adapter_number);
        ...
        nic_combo_box.addActionListener(new ActionListener() { // Event Listener
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    ethernet_src_address.append(getMacAddress(((NILayer) m_layer_mgr.getLayer("NI"))
        ...
        // 방법 2
//	    String[] adapterna= new String[((NILayer) m_layer_mgr.getLayer("NI")).m_adapter_list.size()];
//
//	    for(int i=0;i<((NILayer) m_layer_mgr.getLayer("NI")).m_adapter_list.size();i++)
//	    	adapterna[i] = ((NILayer) m_layer_mgr.getLayer("NI")).m_adapter_list.get(i).getDescription();
//
...
//
//	            try {
//	            	byte[] mac = ((NILayer) m_layer_mgr.getLayer("NI")).m_adapter_list.get(adapter_number).getHardwareAddress();
...
//	            	}
//	            	byte[] ipSrcAddress = ((((NILayer)m_layer_mgr.getLayer("NI")).m_adapter_list.get(adapter_number).getAddresses()).get(0)).getAddr().getData();
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (my_info_setting_button.getText() == "Setting") {
                    ...
                    if (src_ip_string.compareTo("") == 0) {
                        ...
                        ((EthernetLayer) m_layer_mgr.getLayer("Ethernet")).setEnetSrcAddress(ethernet_src);
                        ((ARPLayer) m_layer_mgr.getLayer("ARP")).setHostMacAddr(ethernet_src);
